...
rawdata = rawdata[!,Not([:id, :entry_date, :date_symptoms,:date_died, :patient_type, :icu, :sex])]
...
datawithoutmissing = @linq rawdata |> 
            where(:intubed .!=97, :intubed .!=99,
                    :pneumonia .!=99,
                    :pregnancy .!=97, :pregnancy .!=98,
                    :diabetes .!=98,
                    :copd .!=98,
                    :asthma .!=98,
                    :inmsupr .!=98,
...
models = [simplelogistic, logisticL2, mlp_1layer10nodes, mlp_1layer30nodes,
            mlp_2layer30nodes,mlp_2layer100nodes,mlp_6layer]

function get_prediction(input)
    predict(simplelogistic,[input])
end
...
cross_validated = cross_val_score(LogisticRegression(penalty=:none, max_iter=200), X_train, y_train)